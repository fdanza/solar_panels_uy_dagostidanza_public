...
```{r}
final_sa_6$date <- 
  as.Date(final_sa_6$date)

final_sa_6$true_date_install <- 
  as.Date(final_sa_6$true_date_install)

identical(class(final_sa_6$date), 
          class(final_sa_6$true_date_install))

final_sa_6 <- final_sa_6 %>% 
  mutate(date_num=as.numeric(true_date_install))

#final_sa_6 <- final_sa_6 %>% mutate (cohort = 1)
...
```{r}
# ggsave(paste0(dir$fig,"plot_rob_6.png"),
#        width = 7, height = 4.5)
```
...
# Load the table into R as text
etable_path <- 
  paste0(dir$tab, "sa_6_results.txt")
...
```{r}
final_sa_3$date <- 
  as.Date(final_sa_3$date)

final_sa_3$true_date_install <- 
  as.Date(final_sa_3$true_date_install)

identical(class(final_sa_3$date), 
          class(final_sa_3$true_date_install))

final_sa_3 <- final_sa_3 %>% 
  mutate(date_num=as.numeric(true_date_install))

#final_sa_3 <- final_sa_3 %>% mutate (cohort = 1)
...
# Load the table into R as text
etable_path <- 
  paste0(dir$tab, "sa_3_results.txt")
...
```{r}
main_result_2 <- feols(ext ~  lead12 + lead11 + lead10 + lead9 + lead8 + lead7 + lead6 + lead5 + lead4 + lead3 + lead2 + lag1 + lag2 + lag3 + lag4 + lag5 + lag6 + lag7 + lag8 + lag9 + lag10 + lag11 + lag12 | ID_34 + month, 
                       cluster = "ID_34", data=final_MG)
```
...
```{r}
main_iny_2 <- feols(ener_iny ~  lead12 + lead11 + lead10 + lead9 + lead8 + lead7 + lead6 + lead5 + lead4 + lead3 + lead2 + lag1 + lag2 + lag3 + lag4 + lag5 + lag6 + lag7 + lag8 + lag9 + lag10 + lag11 + lag12 | ID_34 + month, 
                    cluster = "ID_34", data=final_MG)
```
...
```{r}
neteffect_plot <- feols((ext - ener_iny) ~  lead12 + lead11 + lead10 + lead9 + lead8 + lead7 + lead6 + lead5 + lead4 + lead3 + lead2 + lag1 + lag2 + lag3 + lag4 + lag5 + lag6 + lag7 + lag8 + lag9 + lag10 + lag11 + lag12 | ID_34 + month, 
                        cluster = "ID_34", data=final_MG)
```
...
sa_ex_r1 <- feols(ext ~ sunab(date_installation, month_plot)| ID_34 +  month,
                    data = robust_1, cluster = "ID_34")

reg_list_sa_bd = 
  list(summary(sa_ex_r1,
               agg = "ATT"))
...
```{r}
#ID + month
all_years <- feols(ext  ~ treat_2012 + treat_2013 + treat_2014 + treat_2015 + treat_2016 + treat_2017 + treat_2018 + treat_2019 + treat_2020 + treat_2021 + treat_2022| ID_34 + month, cluster = "ID_34", data=interact_treat)
```
...
```{r}
all_years_net <- feols((ext -ener_iny)  ~   treat_2012 + treat_2013 + treat_2014 + treat_2015 + treat_2016 + treat_2017 + treat_2018 + treat_2019 + treat_2020 + treat_2021 + treat_2022| ID_34 + month, cluster = "ID_34", data=interact_treat)
...
rebound_graph <- 
  ggplot(data = 
           rebound_data, aes(x=Months, y=lowerbound, 
                             color="4.3 hrs of peak sunlight")) +
  geom_line(aes(y=upperbound, 
                color="4.5 hours of peak sunlight"), size = 1) +    
  geom_line(size = 1) + 
  theme(text = element_text(size = 15)) + 
  scale_color_manual(name = "", 
                     values = c("4.3 hrs of peak sunlight" = "blue", 
                                "4.5 hours of peak sunlight" = "navy")) + 
