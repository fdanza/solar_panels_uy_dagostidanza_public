...
```{r}
load(file=paste0(dir$data,"Gurobi/final_co2.RData"))
```

Eliminate the variables that I dont need:
```{r}
co2 <- final_co2 %>% 
  dplyr::select(c("year", "month", "day", "prod", "prod_gas", "own_consum", "gasoil_consum", "gas_consum", "gas_spe_consum", "gas_spe_consum_2", "fueloil_consum", "fuelmot_consum", "fuel_oil", "fueloil_tj", "gasoil_tj", "fueloil_co2", "gasoil_co2", "ng_co2", "total_co2", "desc_co2"))
```
...
nov_2018 <- 
  nov_2018 %>% mutate(day=1, hour=0) #create the variable day
nov_2018 <- 
  nov_2018[rep(seq_len(nrow(nov_2018)), each = 30), ] #multiplies each row 30 times
...
```{r}
final_co2_G <- 
  rbind(co2_2018, co2_2019, co2_2020, co2_2021, co2_2022)

save(final_co2_G, 
     file=paste0(dir$data,"Gurobi/final_co2_G.RData"))
```
